export default function ProfileInfo({userName,userTag, userAvatar,userId,component}) {
    return (
        <div id={userId} className="flex">
            <div className="flex w-16 h-16 mr-2">
                <img className="w-14 h-14 rounded-full object-cover" src={`avatars/${userAvatar}`}  alt={`Аватар ${userName}`} />
            </div>
            <div className="flex flex-col">
                <p className={`${component === "post" ? "text-white" :"text-black"} text-2xl`}>{userName}</p>
                <a className={`${component === "post" ? "text-[#C0C0C0]" :"text-black text-opacity-75"} text-sm`}>{userTag}</a>
            </div>
        </div>
    )
}